#!/usr/bin/env python3
"""
iwoozie.baby AI聊天系统 - 纯Python客户演示版本
=============================================
不依赖外部库的纯Python实现，适合任何Python环境
"""
...
class PurePythonDemo:
    ...
    def test_model(self, model, show_details=True):
        ...
        try:
            ...
            status_code, response_data = self.make_request(url, data, headers)
                ...
                try:
                    ...
                    if result.get('code') == 200:
                        ...
                        if show_details:
                            ...
                            message = result.get('data', {}).get('message', 'No message')
    ...
    def test_stream_bypass(self):
        ...
        try:
            ...
            if status_code == 200:
                ...
                print(f"❌ 流式请求失败: HTTP {status_code}")
                self.results['failed'] += 1
